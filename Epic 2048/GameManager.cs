using Willvin.Collections;
using Willvin.DataUtils;
using System;
...
namespace Epic_2048
{
    ...
    sealed class GameManager
    {
                        ...
                        for (int b = a + 1; b < tiles.SideLength; b++)
                        {
                            ...
                            if (tiles[cell2.Row, cell2.Column] != null)
                            {
                                ...
                                if (tileNumber == tiles[cell2.Row, cell2.Column].Number)
                                {
                                    ...
                                    // Check whe the game is won.
                                    if (tileNumber == 2048 && tilesCountPerSide == 4)
                                        won = true;
                                    if (tileNumber == 4096 && tilesCountPerSide == 6)
                                        won = true;
                                    if (tileNumber >= 8192 && tilesCountPerSide == 8)
                                        won = true;
                                }
                                else
                                {
        ...
        public void SetGameLayout(int gameLayoutSelectedIndex)
        {
            ...
            if (gameLayoutSelectedIndex == 0)
                tilesCountPerSide = MinTilesCountPerSide;

            if (gameLayoutSelectedIndex == 1)
                tilesCountPerSide = MinTilesCountPerSide + 2;

            if (gameLayoutSelectedIndex == 2)
                tilesCountPerSide = MinTilesCountPerSide + 4;
